...
2. Go to **Settings → API Tokens** and create a token (e.g. full access or restrict to the content the frontend needs).
...
## 6. Optional: use Cloudinary in Docker
...
| Frontend options (logs, labels) | Set `VITE_ENABLE_SERVER_LOGS` and `VITE_SHOW_PRAYER_DAY_LABELS` to `true` or `false` in root `.env`, then rebuild: `docker compose up -d --build frontend`. |
